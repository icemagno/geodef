...
function makeGrid( latitude, longitude ){
	
	for ( x=3; x<7; x++ ) {
		var geohash = Geohash.encode( latitude, longitude, x );
		addCesiumWall( geohash );		
	}
	
}


function addCesiumWall( geohash ){
	var box = Geohash.bounds( geohash );
	
	var west = box.sw.lon;
	var east = box.ne.lon;
	var north = box.ne.lat;
	var south = box.sw.lat;
	
	
	viewer.entities.add({
		position: Cesium.Cartesian3.fromDegrees( west, north, 2300.0 ),
		label: {
			text: geohash,
			showBackground : true,
	...
	viewer.entities.add({
	    wall : {
	        positions : Cesium.Cartesian3.fromDegreesArrayHeights([west, south, 2000.0,
	                                                        east, south, 2000.0,
	                                                        east, north, 2000.0,
	                                                        west, north, 2000.0,
	                                                        west, south, 2000.0]),
...
// Nao ta funcionando. Corrigir pra pegar o bbox corretamente da nova funcao

/*
function generateGeojsonFeature( geohash ){
	...
	var p1 = [ box.latitude[0], box.longitude[0] ];
	var p2 = [ box.latitude[1], box.longitude[1] ];
	var p3 = [ box.latitude[2], box.longitude[2] ];
	
	feature.geometry = {"type" : "Polygon", "coordinates" : []};
	feature.geometry.coordinates = [ p1, p2, p3, p1 ];	
	
	feature.properties = {};
	feature.properties.geohash = geohash;
