...
function checkUserName( userName ){
	...
	if( $("#name").val() === "" ){
		$("#symbolUserName").removeClass("bg-red");
		return;
	}
	
	$.ajax({
		url: '/userbyname?loginName=' + userName,
		success: function ( result, textstatus ) {
			if ( result === "YES" ){
				userExists = true;
				$("#symbolUserName").addClass("bg-red");
			} else {
				$("#symbolUserName").removeClass("bg-red");
			}
...
function checkCpf( cpf ){
	...
	if( $("#cpf").inputmask('unmaskedvalue') === "" ){
		$("#symbolCpf").removeClass("bg-red");
		return;
	}
	
	if( !testaCPF( $("#cpf").inputmask('unmaskedvalue') ) ){
		$("#symbolCpf").addClass("bg-red");
		cpfValid = false;
		return;
	}
	...
	$( "#password" ).keyup(function() {
		...
		if( password != "" ) {
			...
			$("#passwordBar").css('display','block');
	  	  	...
	  	  	var url="/getPasswordScore?password=" + encodeURIComponent( password );
		  	$.ajax({
		        url      : url,
		        method   : 'GET',
		        success : function (obj, textstatus) {
